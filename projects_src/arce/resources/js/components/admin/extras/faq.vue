				...
				<div class="panel-body">
					<form role="form" class="form-horizontal" @submit.prevent="submit($event.target)">				
						...
						<div class="form-group">
							<label for="editor2" class="col-sm-3 control-label">Contenido</label>
	...
	export default {
		...
		methods:{
			getContent(){
				...
				axios.get(tools.url("/api/admin/faqs")).then((response)=>{
			...
			submit(form){			
				...
				this.$parent.validateAll(()=>{
					...
					axios.post(tools.url("/api/admin/faqs"), data).then((response)=>{
